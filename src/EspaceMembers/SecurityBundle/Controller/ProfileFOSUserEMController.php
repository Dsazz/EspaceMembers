...
class ProfileFOSUserEMController extends Controller
{
    ...
    public function editAuthenticationAction()
    {
        ...
        //if (!is_object($user) || !$user instanceof UserInterface || $securityContext->isGranted('ROLE_USER')) {
        if (!is_object($user) || !$user instanceof UserInterface) {
            throw new AccessDeniedException('This user does not have access to this section.');
        }
    ...
    public function editProfileAction()
    {
        $securityContext = $this->container->get('security.context');
        $user = $securityContext->getToken()->getUser();
        //if (!is_object($user) || !$user instanceof UserInterface || !$user->hasRole('ROLE_ADMIN')) {
        if (!is_object($user) || !$user instanceof UserInterface) {
            throw new AccessDeniedException('This user does not have access to this section.');
        }
